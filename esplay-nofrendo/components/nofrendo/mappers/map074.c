...
static void map4_write_sram(uint32 address, uint8 value)
{
    nes_t* nes = NULL;
    if (address >= 0x6000 && address <= 0x7FFF) 
    {
        nes = nes_getcontextptr();
        nes->cpu->mem_page[address >> 12][address & 0xFFF] = value;
    }
    else
    {
        printf("map74: write: address=%p, value=0x%x\n", (void*)address, value);
    }
}

static uint8 map4_read(uint32 address)
{
    nes_t* nes = NULL;
    if (address >= 0x6000 && address <= 0x7FFF) 
    {
        nes = nes_getcontextptr();
        return nes->cpu->mem_page[address >> 12][address & 0xFFF];
    }
    else
    {
        printf("map74: read: address=%p\n", (void*)address);
        return 0;
    }
}
...
static map_memwrite map4_memwrite[] =
{
   ...
   { 0x6000, 0x7FFF, map4_write_sram },
   {     -1,     -1, NULL }
};

static map_memread map4_memread[] = 
{
    { 0x4020, 0xFFFF, map4_read },
    {-1, -1, NULL}
};
